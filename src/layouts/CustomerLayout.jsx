...
const navItems = [
  ...
  { label: "Ưu đãi", path: "/customer/promotions", icon: BadgePercent },
  { label: "Đổi thưởng", path: "/customer/rewards", icon: Gift },
  ...
  const sidebarContent = (
    <>
      <Link to="/customer" className="flex items-center gap-3 px-2 pb-6">
        ...
        <span>
          <span className="block text-lg font-extrabold tracking-tight text-[var(--text-main)]">WashMate</span>
          <span className="block text-xs font-medium text-[var(--text-muted)]">Cổng khách hàng</span>
        </span>
      </Link>

      <nav className="flex-1 space-y-1 overflow-y-auto px-1">
        {navItems.map(({ label, path, icon: Icon, end }) => (
          <NavLink key={path} to={path} end={end} onClick={() => setMobileNavOpen(false)} className={navLinkClass}>
            <Icon size={18} />
            {label}
          </NavLink>
        ))}
      ...
      <div className="mt-4 border-t border-[var(--border-soft)] pt-4">
        <button type="button" onClick={handleLogout} className="flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold text-[var(--text-muted)] transition hover:bg-red-50 hover:text-red-500">
          <LogOut size={18} />
          Đăng xuất
        </button>
      </div>
      ...
      <div className="flex">
          ...
          <div className="fixed inset-0 z-40 lg:hidden">
            ...
            <aside className="absolute left-0 top-0 flex h-full w-72 flex-col bg-white px-4 py-6 shadow-2xl">
              <div className="mb-2 flex justify-end">
                <button type="button" onClick={() => setMobileNavOpen(false)} className="rounded-full p-2 text-[var(--text-muted)] hover:bg-[var(--bg-main)]" aria-label="Đóng menu">
                  <X size={20} />
                </button>
              </div>
              {sidebarContent}
        ...
        <div className="flex min-h-screen w-full flex-1 flex-col">
          <header className="sticky top-0 z-30 border-b border-[var(--border-soft)] bg-white/95 backdrop-blur">
            <div className="flex items-center gap-4 px-4 py-4 sm:px-6 lg:px-8">
              ...
              <div className="relative max-w-md flex-1">
                ...
                <input type="text" placeholder="Tìm kiếm nhanh..." className="w-full rounded-full border border-[var(--border-soft)] bg-[var(--bg-main)] py-2.5 pl-11 pr-4 text-sm text-[var(--text-main)] outline-none transition focus:border-[var(--brand-blue)] focus:ring-2 focus:ring-[var(--brand-blue)]/15" />
              </div>

              <div className="ml-auto flex items-center gap-3">
                <Link to="/customer/notifications" className="relative flex h-11 w-11 items-center justify-center rounded-full border border-[var(--border-soft)] bg-white text-[var(--text-main)] transition hover:border-[var(--brand-blue)] hover:text-[var(--brand-blue)]" aria-label="Thông báo">
                  <Bell size={18} />
                  <span className="absolute right-2.5 top-2.5 h-2 w-2 rounded-full bg-[var(--brand-blue)]" />
                </Link>
                <Link to="/customer/loyalty" className="hidden items-center gap-2 rounded-full bg-[var(--brand-blue)]/10 px-4 py-2 text-sm font-semibold text-[var(--brand-blue)] sm:flex">
                  <Award size={16} />
                  Điểm thưởng
                </Link>
                <div className="flex items-center gap-3 rounded-full border border-[var(--border-soft)] bg-white px-2 py-1.5 pr-4">
                  <span className="flex h-9 w-9 items-center justify-center rounded-full bg-[var(--brand-blue)] text-sm font-bold text-white">{avatarInitial}</span>
                  <span className="hidden text-sm font-semibold text-[var(--text-main)] md:block">{customerName}</span>
                </div>
              </div>
            </div>
          </header>
          ...
          <main className="flex-1 px-4 py-6 sm:px-6 lg:px-8">
            <div className="mx-auto w-full max-w-[1180px]"><Outlet /></div>
          </main>
        </div>
      </div>
    </div>
