...
export function VehicleStep({ vehicles, selectedId, onSelect }) {
  if (!vehicles.length) {
    return <div className="rounded-3xl border border-dashed border-[var(--border-soft)] bg-white p-10 text-center"><Car className="mx-auto size-10 text-[var(--brand-blue)]" /><h2 className="mt-4 text-xl font-extrabold">Bạn chưa có xe nào</h2><p className="mt-2 text-sm text-[var(--text-muted)]">Hãy thêm xe trước khi tạo lịch chăm sóc.</p><Link to="/customer/vehicles" className="mt-6 inline-flex items-center gap-2 rounded-2xl bg-[var(--brand-blue)] px-5 py-3 text-sm font-bold text-white"><Plus size={17} /> Thêm xe mới</Link></div>;
  }
