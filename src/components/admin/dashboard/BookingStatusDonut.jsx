          ...
          {data.length > 0 ? data.map((d, i) => (
            <div key={i} className="flex items-center justify-between gap-1 text-xs">
              <span className="flex min-w-0 items-center gap-1.5">
                ...
                <span className="truncate font-bold text-muted-foreground">{d.name}</span>
              </span>
              <span className="shrink-0 font-bold text-foreground">{d.value} <span className="font-normal text-neutral-muted">({d.percentage}%)</span></span>
            </div>
          )) : (
