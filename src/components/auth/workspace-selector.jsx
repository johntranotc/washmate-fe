    ...
    <div className="flex min-h-screen flex-col bg-surface">
      ...
      <main className="mx-auto flex w-full max-w-5xl flex-1 flex-col justify-center px-5 py-10 sm:px-8">
        <div className="text-center">
          <span className="inline-flex rounded-full border border-border bg-white px-4 py-2 text-sm font-semibold text-primary">Không gian phù hợp với vai trò của bạn</span>
          ...
          <>
            <div className="mt-10 grid gap-5 md:grid-cols-3">
              {visibleRoles.map((role) => {
                ...
                return <button key={role} type="button" onClick={() => setSelected(role)} className={cn("relative flex min-h-64 flex-col items-start rounded-[2rem] border-2 bg-white p-7 text-left transition", active ? "border-primary shadow-[0_24px_50px_-26px_rgba(11,140,255,.6)]" : "border-border hover:border-primary/40")}><span className={cn("grid size-14 place-items-center rounded-2xl", active ? "bg-primary text-white" : "bg-secondary text-primary")}><Icon className="size-7" /></span><h2 className="mt-6 text-xl font-bold">{option.title}</h2><p className="mt-2 flex-1 text-sm leading-relaxed text-muted-foreground">{option.description}</p><span className={cn("grid size-6 place-items-center rounded-full border-2", active ? "border-primary bg-primary text-white" : "border-border text-transparent")}><Check className="size-3.5" /></span></button>;
              })}
            </div>
            <button type="button" onClick={() => navigate(destinationForRole(selected))} className="mx-auto mt-10 inline-flex h-12 w-full max-w-xs items-center justify-center gap-2 rounded-xl bg-primary font-bold text-white shadow-lg hover:bg-brand-dark">Tiếp tục <ArrowRight className="size-4" /></button>
          </>
        ) : (
          <div className="mx-auto mt-10 max-w-lg rounded-3xl border border-border bg-white p-8 text-center shadow-sm">
