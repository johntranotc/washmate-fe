...
export default function StaffBookingWorkflowPage() {
  ...
  useEffect(() => {
    staffApi.getStaffBookingById(bookingId)
  ...
  return (
    <div className="space-y-6">
      <PageHeader eyebrow="Chi tiết vận hành" title={booking.code} description={`${booking.customerName} · ${booking.vehicle} · ${booking.plate}`} actions={<StatusBadge status={booking.bookingStatus} />} />

      {booking.bookingStatus === "PENDING" && <div className="flex gap-3 rounded-2xl border border-warning/25 bg-warning-container p-4 text-sm text-warning"><CircleAlert size={18} />Lịch này đang chờ gara xác nhận nên chưa thể check-in.</div>}
      <div className="grid gap-5 lg:grid-cols-[1fr_360px]">
        <section className="space-y-5 rounded-2xl border border-border bg-card p-6">
          ...
          ].map(([label, value]) => <div key={label} className="rounded-xl bg-surface p-4"><p className="text-xs text-neutral-muted">{label}</p><b className="mt-1 block text-sm">{value}</b></div>)}</div>
          <div><p className="text-xs font-bold">Ghi chú</p><p className="mt-2 rounded-xl border border-border p-4 text-sm text-muted-foreground">{booking.note || "Không có ghi chú."}</p></div>
          <div><h2 className="font-extrabold">Dòng thời gian trạng thái</h2><div className="mt-4 grid gap-3 sm:grid-cols-3">{[["Check-in", booking.checkinTime], ["Bắt đầu rửa", booking.serviceStartTime], ["Hoàn tất", booking.completedTime]].map(([label, value]) => <div key={label} className="rounded-xl border border-border p-4"><Clock3 className="text-primary" size={16} /><p className="mt-2 text-xs font-bold">{label}</p><p className="mt-1 text-xs text-muted-foreground">{value ? new Date(value).toLocaleString("vi-VN") : "Chưa ghi nhận"}</p></div>)}</div></div>
        </section>
